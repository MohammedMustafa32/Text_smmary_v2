...
model = SimpleT5()
model.load_model("t5","simplet5_model")

def generate_summary_simplet5(text):
...
def get_summary():
    ...
    final_text = generate_summary_simplet5(raw_text)
    print(final_text)
...
def get_text():
    ...
    if response.status_code == 200:
        ...
        print("Error: Could not retrieve text from URL")
...
def get_url_summary():
    ...
    raw_text = pre_processing(raw_text)
    final_text = generate_summary_simplet5(raw_text)
    result = '\nSummary:{}'.format(final_text)
